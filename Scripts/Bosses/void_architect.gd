...
func perform_attack(sequence: int) -> void:
	if not is_instance_valid(GameManager.player):
		return
	var aim_angle := global_position.direction_to(GameManager.player.global_position).angle()
	...
	if current_phase >= 3:
		spawn_circle_hazard(GameManager.player.global_position, 150.0, 1.0, 42.0, definition.secondary_color)
