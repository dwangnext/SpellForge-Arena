...
func configure(title: String, position: Vector2, health: float, attack_damage: float, uses_range: bool, color: Color, ship := false, tier := 0) -> void:
	display_name = title
...
func _attack(target: Node2D) -> void:
	if ranged:
		...
		projectile.configure(global_position, global_position.direction_to(target.global_position), damage, accent_color, ship_tier)
...
func get_network_actor_data() -> Dictionary:
	return {
		...
		"health": get_health_ratio(), "space_tier": ship_tier,
	}
...
func _draw() -> void:
	...
	if is_ship:
		...
		draw_circle(Vector2(-radius * 0.35, 0), 4.0 + ship_tier * 0.5, Color("9ffcff"))
	else:
